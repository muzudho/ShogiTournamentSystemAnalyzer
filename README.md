...
これは、コンピューターソフトが対局主体になる大会だけでなく、人間が対局主体になる大会にも使えるようにするためです。  
たとえばコンピューター将棋大会では、実際に対局するのはソフトであり、人間は「参加者」です。そうした事情を承知した上で、このツールでは大会システム分析用の抽象化として **選手（Player）** という言葉を使っています。
