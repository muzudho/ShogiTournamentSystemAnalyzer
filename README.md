...
4. `On` のときだけグループ対応CSV
5. `On` のときだけ本戦不出場Apex一覧CSV（省略可）
6. `On` のときだけ本戦不出場Apexの扱い（Off / On）
7. `On` のときだけ境界救済戦の有無（Off / On）
8. `On` かつ品質評価モードのときだけ可変定員8ルールの有無（Off / On）
...
- Apex / Innov 分け方が `Off` のときはニュートラルな対局表として全参加者をフラットに順位づけする
