...
通常の空行および連続する空行は読み飛ばします。空行はセクションの開始・終了・CSV入力終了を表しません。セクションの範囲は `#[Section]` と `#[EndSection]` だけで決まります。

CSV / 複合入力セクション内の空行も同様に読み飛ばします。見た目を整えるために空行を入れても、CSV の空レコードとしては扱いません。
