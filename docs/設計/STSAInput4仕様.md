...
`STSAInput/4` は `STSAInput/2` と同じセクション構文を使います。通常の空行および連続する空行は読み飛ばし、セクションの開始・終了・CSV入力終了としては扱いません。

CSV / 複合入力セクション内の空行も読み飛ばします。セクションの範囲は `#[Section]` と `#[EndSection]` だけで決まります。
