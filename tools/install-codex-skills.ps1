...
# ファイルのコピー先へアクセスできるかテストしてから、作業を始めるぜ（＾▽＾）
if (Test-Path -LiteralPath $destinationSkill) {
    ...
    # 上書き禁止のファイルを強制的に上書きするんなら、古いファイルは名前を変えてバックアップしておこうぜ（＾～＾）
...
# このリポジトリーに置いてある［スキル］ファイルを、他のリポジトリー担当のコーデックスも読めるローカルフォルダーへ、コピーするぜ（＾▽＾）
