...
using ShogiTournamentSystemAnalyzer.Compatibility;
...
internal static class ApplicationTournamentUser
{
    ...
    {        //　　｜
        ...
        if (argumentResult.HasRequestFile)
        {
                ...
                && parsedAnalysisRequest is not null)
            {
                ...
                ruleProfileAttributes = parsedAnalysisRequest.StepRequests[0].GetRuleProfileAttributes();
            }
            else
            {
    ...
    static bool TryConvertToLegacyInputText(RequestText? checkedRequestText, out string? requestText)
    {
        ...
        try
        {
            requestText = RequestFileCompatibilityFacade.ConvertToLegacyInputText(checkedRequestText);
            return true;
        }
