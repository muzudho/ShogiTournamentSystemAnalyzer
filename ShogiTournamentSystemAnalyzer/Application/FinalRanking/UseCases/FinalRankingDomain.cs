...
using ShogiTournamentSystemAnalyzer.Application.TournamentFinalState.UseCases;
...
internal static class FinalRankingDomain
{
        ...
        switch (input.Kind)
        {
            case FinalRankingDomainInputKind.StandardSimulation:
                WriteStandardSimulationOutputs(
                    input.TournamentFinalState ?? throw new InvalidOperationException("通常シミュレーションの大会最終状態がありません。"),
                    input.FirstPlayerWinRatePercent,
                    input.FinalRankingResult ?? throw new InvalidOperationException("通常シミュレーションの最終順位付け結果がありません。"),
                    input.OutputPath);
                return true;

            case FinalRankingDomainInputKind.FinalStageSimulation:
                WriteFinalStageSimulationOutputs(
                    input.TournamentFinalState ?? throw new InvalidOperationException("本戦シミュレーションの大会最終状態がありません。"),
                    input.FirstPlayerWinRatePercent,
                    input.FinalRankingResult ?? throw new InvalidOperationException("本戦シミュレーションの最終順位付け結果がありません。"),
                    input.OutputPath,
                    input.Players,
            ...
            case FinalRankingDomainInputKind.TournamentFrameworkSimulation:
                WriteTournamentFrameworkSimulationOutputs(
                    input.OutputPath,
                    input.TournamentFrameworkFinalRankingResult ?? throw new InvalidOperationException("大会進行フレームワークの最終順位付け結果がありません。"));
                return true;

            case FinalRankingDomainInputKind.EmptySimulation:
                WriteEmptySimulationOutputs(input.EmptyFinalRankingInput ?? new EmptyFinalRankingDomainInput(input.OutputPath));
                return true;
    ...
    internal static void WriteEmptySimulationOutputs(EmptyFinalRankingDomainInput input)
    {
        ...
        var requestedOutputPath = string.IsNullOrWhiteSpace(input.OutputPath)
            ? ConsolePromptReaders.ReadTextWithDefault(
