...
- このリポジトリでは、最終的に大会ルールは一覧選択ではなく DSL ファイル入力へ寄せる方針で、現在は標準ルールと本戦ルールの抽象化をその足場として進めている。
- 作業中にソースコードへ TODO コメントを書き足して要求を伝える運用をしており、その TODO を実装要件として扱うことを望んでいる。
- このプロジェクトでは、［最終順位という境界］にある『次回調整のヒント』や『次回の具体設定案』を残す方針を好む。さらに、今回の調整で使った入力パラメーターのログを［大会ルールという境界］側にも出力し、そのログへのリンクを貼る案を好む。
